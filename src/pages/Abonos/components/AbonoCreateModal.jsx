...
// ── Modal principal ────────────────────────────────────────────────────────
export default function AbonoCreateModal({ isOpen, onClose, formData, setFormData, onSelectChange, onSubmit, loading }) {
    const [tipoAbono, setTipoAbono] = useState("credito"); // "credito" | "contado"
    ...
    const fetchOrders = async (idCliente) => {
        ...
        setOrders([]);
        setSelectedOrder(null);
        onSelectChange("idPedido", "");
        try {
            const res = await fetch(`${API}/orders`, { headers: hdrs() });
            const data = await res.json().catch(() => []);
            const list = Array.isArray(data) ? data : (data.data ?? data.orders ?? []);
            
            // Filtros: permitir cualquier estado operativo y bloquear solo anulados o saldo en cero
    ...
    const handleSelectCreditClient = (c) => {
        setSelectedCredit(c);
        setSelectedOrder(null);
        onSelectChange("idCliente", String(c.idCliente));
        onSelectChange("clienteNombre", c.displayName);
        onSelectChange("idCredito", String(c.idCredito));
        onSelectChange("idPedido", "");
        fetchOrders(c.idCliente);
    };
    ...
    const handleSelectOrder = (o) => {
        ...
        setSelectedOrder(o);
        onSelectChange("idPedido", String(pedidoId));
    };
    ...
    // La validación económica del monto debe evaluar el saldo pendiente del pedido seleccionado de manera estricta.
    const maxAbono = hasSelectedOrder
        ? getOrderPendingBalance(selectedOrder)
        : null;

    const montoOk   = montoPositivo && (maxAbono === null || montoNum <= (maxAbono + 0.5)); // Margen para redondeo
    ...
    const canSubmit = clientOk && formData.metodoPago && montoOk && !selectedOrderIsPaid;

    const handleSubmit = async () => {
        console.log("[AbonoCreateModal] Intento de envio", {
            selectedOrder,
            selectedOrderId,
            resolvedOrderId,
            montoIngresado: formData.montoAbono,
    ...
    // Cambiar tipo resetea selecciones
    const handleTipo = (v) => {
        setTipoAbono(v);
        setSelectedCredit(null); setSelectedClient(null); setSelectedOrder(null); setOrders([]);
        onSelectChange("idCliente", ""); onSelectChange("idCredito", ""); onSelectChange("idPedido", "");
    };
    ...
    return (
        <BaseFormModal isOpen={isOpen} onClose={onClose} title="Registrar Recaudo de Cartera"
            subtitle="Selecciona el tipo de abono y completa los datos del pago."
            icon={HandCoins} loading={loading} isSubmitDisabled={!canSubmit} onSubmit={handleSubmit} maxWidthClass="sm:max-w-[640px]">
                ...
                {tipoAbono === "credito" ? (
                    <div>
                        ...
                        <Select
                            ...
                            onValueChange={(val) => {
                                const c = creditClients.find(item => item.idCliente?.toString() === val);
                        ...
                        {/* Pedidos del cliente (NUEVO para crédito) */}
                        {selectedCredit && (
                            <div className="mt-4">
                                <Label className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-slate-700 dark:text-zinc-300">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <Select
                                value={formData.idCliente?.toString() || ""}
                                onValueChange={(val) => {
                                    const c = allClients.find(item => item.idCliente?.toString() === val);
                ...
                <div className={`grid grid-cols-2 gap-x-5 gap-y-4 transition-opacity ${!clientOk ? "pointer-events-none opacity-40" : ""}`}>
                    <div className="col-span-2 md:col-span-1">
                        <Label className="mb-1.5 block text-xs font-semibold text-slate-700 dark:text-zinc-300">
                            {tipoAbono === "credito" ? "3" : selectedClient ? "4" : "3"}. Monto del Abono <span className="text-emerald-500">*</span>
                        </Label>
                        <div className="relative">
                            ...
                            <Input type="number" min="0" step="0.01" placeholder="Ej: 1500.00"
                                value={formData.montoAbono} onChange={e => onSelectChange("montoAbono", e.target.value)}
                                disabled={loading || !clientOk}
                    ...
                    <div className="col-span-2 md:col-span-1">
                        ...
                        <Select value={formData.metodoPago} onValueChange={v => onSelectChange("metodoPago", v)} disabled={loading || !clientOk}>
                    ...
                    <div className="col-span-2">
                        ...
                        <Input type="text" placeholder="Ej: REC-123456" value={formData.referencia}
                            onChange={e => onSelectChange("referencia", e.target.value)}
                            disabled={loading || !clientOk} className="h-11 rounded-xl border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-slate-900 dark:text-white focus-visible:ring-[#10b981]" />
                    </div>
                    ...
                    <div className="col-span-2">
                        ...
                        <Textarea value={formData.descripcion} onChange={e => onSelectChange("descripcion", e.target.value)}
                            placeholder="Ej: 'pago parcial factura #33'…" disabled={loading || !clientOk}
