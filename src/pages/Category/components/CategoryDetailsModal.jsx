...
const CategoryDetailsModal = ({ isOpen, onClose, categoria }) => {
  ...
  const categoryName = categoria.nombre || categoria.nombre_categoria || "Categoria";
        ...
        <div className="p-6 space-y-6 bg-white dark:bg-zinc-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <InfoCard icon={Hash} iconColor="blue" title="Información Técnica">
              <div>
                <p className="text-xs text-slate-500 dark:text-zinc-450 mb-1">ID de Categoría</p>
              ...
              <div>
                <p className="text-xs text-slate-500 dark:text-zinc-450 mb-1">Slug Interno</p>
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                  cat_{categoryName.toLowerCase().replace(/\s+/g, "_")}
                </p>
              </div>
            </InfoCard>
            ...
            <InfoCard icon={Info} iconColor="violet" title="Gestión">
              <div>
                <p className="text-xs text-slate-500 dark:text-zinc-450 mb-1">Estado Operativo</p>
                <p className={`text-sm font-bold ${categoria.statusId === 1 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-zinc-500'}`}>
                  {categoria.statusId === 1 ? "Habilitado para Inventario" : "Restringido / Inactivo"}
                </p>
              </div>
              <Separator className="my-2 dark:bg-zinc-800" />
              <div>
                <p className="text-xs text-slate-500 dark:text-zinc-450 mb-1">Productos Asociados</p>
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                  Cálculo pendiente...
                </p>
              </div>
